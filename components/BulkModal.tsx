...
const BulkModal: React.FC<BulkModalProps> = ({
  ...
  useEffect(() => {
    if (initialTagOptions && initialTagOptions.length > 0) {
      console.log('📝 Updating BulkModal tagOptions:', initialTagOptions.length);
  ...
  const filteredAgents = useMemo(() => {
    ...
    if (currentUser.roleRef && typeof currentUser.roleRef.isSalesRole === 'boolean') {
    ...
    if (isSalesRole) {
      return agents.filter((agent) => agent.value !== currentUser.id && agent.value !== currentUser._id);
    }
  ...
  // Debug options data
  useEffect(() => {
    console.log('🐛 BulkModal Options Debug:', {
      statusOptions: statusOptions?.length || 0,
      sourceOptions: sourceOptions?.length || 0,
      agents: agents?.length || 0,
      initialTagOptions: initialTagOptions?.length || 0,
      currentTagOptions: tagOptions?.length || 0,
      visible,
    });
    
    if (initialTagOptions && initialTagOptions.length > 0) {
      console.log('📝 First few tag options:', initialTagOptions.slice(0, 3));
    }
  }, [statusOptions, sourceOptions, agents, initialTagOptions, tagOptions, visible]);
  ...
  const handleChange = useCallback((field: string) => (value: any) => {
    if (field === 'assignee') {
      ...
      if (currentUser) {
        ...
        if (isSalesRole && (currentUser.id === value || currentUser._id === value)) {
      ...
      const selectedSource = sourceOptions.find((source) => source.value === value);
      setBulkData((prev) => ({
        ...prev,
  ...
  const toggleBooleanField = useCallback((field: keyof BulkData) => () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      Toast.show(err.message || 'An error occurred', {
        duration: Toast.durations.LONG,
      });
    } finally {
      setLoading(false);
    ...
    >
      <View className="flex-1 bg-gray-50">
        ...
        <View className="flex-row items-center justify-between p-4 bg-white border-b border-gray-200">
          ...
          <View className="w-6" />{/* Spacer for center alignment */}
        </View>
            ...
            <View className="gap-6">
              ...
              <View>
                ...
                <View className="flex-row gap-6 mt-3">
                  ...
                  >
                    ...
                    <Text className="text-sm text-miles-600">Add selected tags</Text>
                  </TouchableOpacity>
                  ...
                  >
                    ...
                    <Text className="text-sm text-red-600">Remove selected tags</Text>
                  </TouchableOpacity>
                </View>
              ...
              <View>
                ...
                >
                  ...
                  <Text className="text-sm text-gray-600">Replace existing description</Text>
                </TouchableOpacity>
              </View>
                ...
                <View>
                  ...
                  <View className="flex-row items-center ml-6 mt-1">
                    ...
                    <Text className="ml-1 text-xs text-gray-500">
                      Allows reassignment when leads have future meetings or reminders
                    </Text>
                  </View>
                </View>
              </View>
                ...
                <View className="border border-amber-200 rounded-lg bg-amber-50 p-4">
                  <View className="flex-row justify-between items-start mb-2">
                    ...
                    <TouchableOpacity className="bg-miles-50 px-3 py-2 rounded-lg">
                      <Ionicons name="download-outline" size={16} color="#176298" />
                    </TouchableOpacity>
                  </View>
                  <ScrollView className="max-h-32" showsVerticalScrollIndicator={false}>
                    {skippedLeads.map((lead) => (
                      <Text key={lead._id} className="text-sm text-amber-700 mb-1">
